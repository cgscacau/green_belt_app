...
def initialize_firebase():
    ...
    try:
        # Verificar se já foi inicializado - SEM DEBUG
        if firebase_admin._apps:
            return firestore.client()
        
        # Obter Project ID primeiro
        project_id = get_project_id()
        if not project_id:
            return None
        
        # Definir variável de ambiente para Google Cloud
        ...
        # Método 4: Application Default Credentials
        if not cred:
            try:
        ...
        # Fallback: apenas com Project ID
        try:
            app_options = {'projectId': project_id}
